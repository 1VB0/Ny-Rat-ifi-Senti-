...
// Fix: state-indicator dots now OFF by default (toggle in inputs)
// ─────────────────────────────────────────────────────────────────────────────
//@version=6
...
div_memory   = input.int   (20,   "Divergence Memory (bars)",    minval = 1, maxval = 50,
     tooltip = "How many bars after detecting RSI divergence it stays active as a filter.")
retest_tol   = input.float (1.0,  "Retest Tolerance (ticks)",    minval = 0., maxval = 10., step = 0.5,
     tooltip = "How many ticks inside the 5m range boundary counts as a valid retest.")
show_labels  = input.bool  (true, "Show Long/Short Labels")
show_ranges  = input.bool  (true, "Show Range Lines")

// ── State dots toggle — OFF by default (was causing the red dot flood) ───────
show_state_dots = input.bool(false, "Show State Indicator Dots",
     tooltip = "Small circles/triangles that show when a setup is in state ① (broke) or ② (retested). OFF by default to keep the chart clean.")
...
snap_930  = f_snapshot("0830-0831:23456")
snap_935  = f_snapshot("0835-0836:23456")
snap_940  = f_snapshot("0840-0841:23456")
snap_945  = f_snapshot("0845-0846:23456")
snap_1000 = f_snapshot("0900-0901:23456")
snap_1030 = f_snapshot("0930-0931:23456")
snap_1330 = f_snapshot("1230-1231:23456")
snap_1800 = f_snapshot("1600-1601:23456")

in_ny = not na(time("1", "0830-1900:23456"))
...
// ─────────────────────────────────────────────────────────────────────────────
// SIGNAL SHAPES — LONG / SHORT labels only
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// STATE DOTS  — gated behind show_state_dots (default OFF)
//  These were the source of the red dot flood on the chart.
//  bull_state == 1 / bear_state == 1 fires on almost every bar outside
//  the range, producing a continuous stream of dots.  Toggle ON only
//  when you need to debug the state machine.
// ─────────────────────────────────────────────────────────────────────────────
plotshape(show_state_dots and bull_state == 1 and not long_signal,
     title    = "Bull Setup Active",
     style    = shape.circle,
     location = location.belowbar,
...
alertcondition(bull_state == 2,"Bull Retest Zone Active","5m hi_5m has been retested — watching for long re-break")
alertcondition(bear_state == 2,"Bear Retest Zone Active","5m lo_5m has been retested — watching for short re-break")
...
if barstate.islast
    ...
    table.cell(leg, 1, 1, "8:35",  text_color = color.white, text_size = size.tiny)
    ...
    table.cell(leg, 1, 2, "8:40",  text_color = color.white, text_size = size.tiny)
    ...
    table.cell(leg, 1, 3, "8:45",  text_color = color.white, text_size = size.tiny)
    ...
    table.cell(leg, 1, 4, "9:00",  text_color = color.white, text_size = size.tiny)
    ...
    table.cell(leg, 1, 5, "9:30",  text_color = color.white, text_size = size.tiny)
    ...
    table.cell(leg, 1, 6, "12:30p",text_color = color.white, text_size = size.tiny)
    table.cell(leg, 2, 6, "",      bgcolor = #ca97ee,         text_size = size.tiny)

    table.cell(leg, 0, 7, "8H",    text_color = color.white, text_size = size.tiny)
    table.cell(leg, 1, 7, "4:00p", text_color = color.white, text_size = size.tiny)
    ...
    table.cell(leg, 0, 9,  "Bull",     text_color = color.silver,  text_size = size.tiny, bgcolor = sbg)
    table.cell(leg, 1, 9,  bull_st_txt,text_color = bull_st_col,   text_size = size.tiny, bgcolor = sbg)
    table.cell(leg, 2, 9,  "",         bgcolor = bull_st_col,       text_size = size.tiny)

    table.cell(leg, 0, 10, "Bear",     text_color = color.silver,  text_size = size.tiny, bgcolor = sbg)
    table.cell(leg, 1, 10, bear_st_txt,text_color = bear_st_col,   text_size = size.tiny, bgcolor = sbg)
    table.cell(leg, 2, 10, "",         bgcolor = bear_st_col,       text_size = size.tiny)
    ...
    table.cell(leg, 0, 11, "SL / Entry",text_color = color.silver, text_size = size.tiny, bgcolor = sbg)
    table.cell(leg, 1, 11, sl_txt,      text_color = sl_col,        text_size = size.tiny, bgcolor = sbg)
    table.cell(leg, 2, 11, en_txt,      text_color = en_col,        text_size = size.tiny, bgcolor = sbg)
